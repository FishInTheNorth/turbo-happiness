...
<script type="text/javascript">
    <c:if test="${param.f=='1'}">${"window.alert(\"原密码不能与新密码一致\");"}</c:if>
    <c:if test="${param.f=='2'}">${"window.alert(\"修改密码与确认密码不服\");"}</c:if>
    <c:if test="${param.f=='3'}">${"window.alert(\"原密码错误\");"}</c:if>
    function showTime() {
        nowtime = new Date();
    ...
    <div class="page-wrapper mdc-toolbar-fixed-adjust">
        <main class="content-wrapper">
            <div class="mdc-layout-grid__cell mdc-layout-grid__cell--span-12-desktop">
                ...
                <form action="/adminChangPassword" method="post">
