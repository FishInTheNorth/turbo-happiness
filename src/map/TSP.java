...
//先让老师选择自己的城市，然后把所有城市存在List中，去重后得到新的List。再进行SQL语句的分别经纬度查询，两个hashmap，
//一个存经度一个存纬度，前面的Key用固定长度的数组。求得了两个hashmap后
//选择进行高德地图的遍历，求出距离，这些距离存在一个二维数组中，二维数组再用TSP求解。一个注意的就是数字与城市的对应，在去重后就应写出对应关系并在jsp中显示出来。
public class TSP {
